...
test('workflow nodes use endpoint and feature color groups consistently', () => {
    ...
    const featureIconRule = readCSSRule(css, '.workflow-node-feature .workflow-node-icon');
    ...
    const featureLabelRule = readCSSRule(css, '.workflow-node-feature .workflow-node-label');
    assert.match(featureLabelRule, /color:\s*var\(--accent\)/);

    const authSubRule = readCSSRule(css, '.workflow-node-feature .workflow-node-sub');
...
test('workflow id pill expands on hover and turns green with a clipboard icon after copy', () => {
    ...
    const placeholderRule = readCSSRule(css, '.workflow-pipeline-meta-placeholder');
    ...
    const iconRule = readCSSRule(css, '.workflow-pipeline-meta-copied .workflow-pipeline-meta-icon');
...
test('AI node renders as a text-only card without an icon', () => {
    ...
    assert.doesNotMatch(template, /class="workflow-node workflow-node-ai[^"]*"[^>]*>\s*<div class="workflow-node-icon">/);
...
test('workflow cards reuse the extracted workflow chart template', () => {
    ...
    assert.match(template, /{{template "workflow-chart" "workflowChart\(workflowPreview\(\)\)"}}/);
    assert.match(template, /{{template "workflow-chart" "workflowChart\(workflow\)"}}/);
    assert.match(template, /{{template "workflow-chart" "workflowAuditChart\(entry\)"}}/);
});
...
test('endpoint pills use dedicated flush-left icons and tighter right padding', () => {
    ...
    assert.match(template, /class="workflow-node-icon workflow-node-icon-endpoint"/);
